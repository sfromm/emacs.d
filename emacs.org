...
#+end_src
** Modeline
For a long time, I've used [[https://github.com/seagle0128/doom-modeline][doom-modeline]].  The goal has been to keep it simple.  In that vein, this is a pivot to try [[https://github.com/emacsmirror/mood-line][mood-line]].
...
| C-x t G | ~tab-bar-change-tab-group~   | Set name for tab group         |
| C-x t g | ~my-tab-bar-switch-to-group~ | Switch to tab group            |
...
*** Toggle deleting other windows
Via Purcell, this is a small wrapper using =winner-mode=.  It toggles whether all other windows are deleted or restores the previously deleted windows.
...
Use the built-in =Abbrevs= mode for abbreviations and snippets.
...
- [[https://clojure.org/guides/learn/clojure][Learn Clojure]]  
...
docker-compose files.  This also pulls in [[https://github.com/Silex/docker.el][docker.el]] for interacting with Docker/Podman.  See also:
...
#+begin_src emacs-lisp :tangle lisp/init-docker.el :noweb yes
...
#+begin_src emacs-lisp :tangle no
(setopt gnus-alias-identity-alist
        '(("personal" ;; Refers-to name
           nil        ;; Refers-to name
           "My Name <example@example.com>"  ;; From email address
           nil        ;; organization header
           nil        ;; extra headers
           nil        ;; body text just above signature
           "~/.signature"))) ;; signature or signature file
#+end_src

And here is an example of =gnus-alias-identity-rules=.  These rules are used to
choose an Identity whenever a new message is created.

#+begin_src emacs-lisp :tangle no
(setopt gnus-alias-identity-rules
        '(("Personal"                   ;; Domain match
           ("To" "example.com" current) ;; Header match: Header symbol, Regexp, Header set
           "personal")))                ;; Identity
#+end_src
...
Something to consider for an org document:

#+begin_example
#+OPTIONS: toc:nil email:t timestamp:nil num:nil author:nil
#+REVEAL_INIT_OPTIONS: width:1200, height:900
