    ...
    (progn
      (setq
        ...
        '(
           ...
           (:name "today"    :key "t" :query "date:today..now and not tag:archive")
           (:name "7 days"   :key "7" :query "date:7d..now and not tag:archive")
           (:name "announce" :key "a" :query "tag:announce and not tag:archive and tag:unread")
    ...
    (progn
      ...
      (when (not (string= system-name sf/work-workstation))
        (setq
          ...
          mu4e-refile-folder "/Personal/Archives")   ;; saved messages
