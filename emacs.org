    ...
    (progn
      (setq elfeed-feeds
        '(("http://codeascraft.com/feed/" devops)
           ...
           ("http://packetpushers.net/category/blogs/feed/" netops)
...
#+BEGIN_SRC emacs-lisp
  (use-package magit
    ...
    (progn
      ...
      (dolist (mode '(magit-mode magit-popup-mode))
        (evil-set-initial-state mode 'emacs))
      )
...
Let's configure ~gnus~ when the command is run.  Pull in [[http://www.gnu.org/software/emacs/manual/html_node/gnus/nnir.html][nnir]] to enable
search on *Gnus* backends.  Then, enable the /gnus-registry/.
