      ...
      (setq
        ...
        org-capture-templates '(
                                 ("j" "Journal" entry (file+datetree (concat org-directory "/journal.org"))
                                   "* %?%U\n")
                                 ("b" "Bookmark" entry (file+headline (concat org-directory "/startpage.org") "Potpourri")
    				     "* %? %^L %^g \n:PROPERTIES:\n:CREATED: %U\n:END:\n\n" :prepend t)
...
#+BEGIN_SRC emacs-lisp
  (use-package elfeed
    ...
    :config
    (progn
      (defun sf/elfeed-show-starred ()
        "Show starred elfeed articles"
        (interactive)
        (elfeed-search-set-filter "@6-months-ago +starred"))

      (defalias 'sf/elfeed-toggle-starred (elfeed-expose #'elfeed-search-toggle-all 'starred))
    ...
    :bind (:map elfeed-search-mode-map
            ("f" . sf/elfeed-toggle-starred)
            ("F" . sf/elfeed-show-starred))
    )
#+END_SRC
    ...
    (progn
      (setq
        ...
        '(
           ...
           (:name "today"           :key "t" :query "date:today..now and not ( tag:archive or tag:sent )")
           (:name "3 days"          :key "3" :query "date:3d..now and not ( tag:archive or tag:sent )")
           (:name "7 days"          :key "7" :query "date:7d..now and not ( tag:archive or tag:sent ) ")
           ...
           (:name "ops/reports"     :key "r" :query "folder:Work/ops and not tag:archive")
