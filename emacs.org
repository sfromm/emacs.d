...
** Bootstrap straight
Go ahead and set up [[https://github.com/raxod502/straight.el][straight.el]] so that we can use it to manage Emacs packages.

Some things to note:

- I move =straight-base-dir= to be under =forge-state-dir= to keep =~/.emacs.d/= tidy.
- Following /Doom Emacs/, I set =straight-build-dir= to be based on =emacs-version=.
- Help =use-package= to use =straight=.
- For now, leave =straight-check-for-modifications= as =nil=.  I may later want
  to consider using =check-on-save=.  See [[https://github.com/raxod502/straight.el#summary-of-options-for-package-modification-detection][docs]].
- Something to keep in mind when using =straight= is that, when using with
  =use-package=, consider whether you are installing and configuring a package
  or configuring a feature.  See:
  + https://github.com/raxod502/straight.el#could-not-find-package-in-recipe-repositories
...
** Straight lockfile
With =straight=, you can now run ~M-x straight-freeze-versions~ to write the
file =straight/versions/default.el=.  The contents of this can then be under
version control -- even in Org and tangled back out to that file.  With this,
you can then restore package versions using ~M-x straight-thaw-versions~.
...
Note that I'm still using =straight= to install and manage packages.
...
Configure Emacs to use the above defined paths.  Also configure some basic behavior
#+begin_src emacs-lisp
...
These are probably not used as much now that I migrated to =straight=.
...
#+end_src
** LIN
[[https://gitlab.com/protesilaos/lin.git][LIN]]
...
#+end_src
** Undo
For now, I have disabled [[https://www.emacswiki.org/emacs/UndoTree][undo-tree]] and am working with the default [[https://www.gnu.org/software/emacs/manual/html_node/emacs/Undo.html][Undo]] system
built-in to Emacs.  Undo is a chain and you can go forwards and backwards, that
is, you can undo and redo.
...
** Docker
[[https://github.com/spotify/dockerfile-mode][dockerfile-mode]] for editing Dockerfiles.
...
* Applications
** Calendar
The calendar functionality is built-in.  This is to configure the /diary/ path.
...
#+end_src
*** COMMENT Org Mode, Notmuch and mail
#+begin_src emacs-lisp
...
(use-package org-mime
  :hook (message-mode . (lambda () (local-set-key "\C-c\M-o" 'org-mime-htmlize)))
  :init
