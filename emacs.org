...
A note regarding terminology: Gnus uses the term /archiving/ to describe
the method for storing mail you send.

There is a lot to digest in the configuration below:

- There is documentation on how to configure the [[http://www.gnus.org/manual/big-gnus.html#Summary-Buffer-Lines][summary line]] fand [[http://www.gnus.org/manual/big-gnus.html#Group-Line-Specification][group line]] format.
- Accounts are configured via =gnus-secondary-select-method=.  The
  default is ~nil~.
- [[https://www.gnu.org/software/emacs/manual/html_node/gnus/Posting-Styles.html][Posting styles]] are discuss what =gnus-posting-sytles= hopes to accomplish.
- There is also  [[http://www.gnu.org/software/emacs/manual/html_node/gnus/Group-Parameters.html][group parameters]] for [[http://www.gnu.org/software/emacs/manual/html_node/gnus/Posting-Styles.html#Posting-Styles][posting styles]] and [[https://www.gnu.org/software/emacs/manual/html_node/gnus/Expiring-Mail.html#Expiring-Mail][expiring mail]].
- Enable the [[https://www.gnu.org/software/emacs/manual/html_node/gnus/The-Gnus-Registry.html][Gnus Registry]] and finally pull in all the above pieces.

~gnus~, by default, will only show new emails.  You should use =C-u RET=
to open a group so that t will show all messages (old and new).
 ...
 #+BEGIN_SRC emacs-lisp 
   (use-package gnus
     ...
     (progn
       ...
       (setq
         ...
         `((nnimap "Work"
             (nnimap-stream shell)
             (nnimap-shell-program "/usr/libexec/dovecot/imap -o mail_location=maildir:~/.mail/Work:INBOX=~/.mail/Work/INBOX:LAYOUT=fs")
             (nnir-search-engine imap)
             )
            (nnimap "Personal"
              (nnimap-stream shell)
              (nnimap-shell-program "/usr/libexec/dovecot/imap -o mail_location=maildir:~/.mail/Personal:INBOX=~/.mail/Personal/INBOX:LAYOUT=fs")
              (nnir-search-engine imap)
              )
            )
         gnus-parameters '(
                            ...
                            ("Personal.*"
                              ...
                              (posting-style
                                ...
                                ("X-Message-SMTP-Method" "smtp smtp.gmail.com 465 sfromm")))
                            ("Work:INBOX"
                              (display . all)
       ...
       (setq
         gnus-agent nil
         gnus-directory (expand-file-name (concat user-emacs-directory "user/gnus"))
         ...
         gnus-visible-headers (mapcar  #'(lambda (header) (format "^%s:" header))
                                (split-string (mapconcat 'identity
                                                '("From Organization Subject Newsgroups"
                                                   ...
                                                   "X-Spam-Level" "Date") " ")))
         nnmail-extra-headers '(To Cc Newsgroups Content-Type Thread-Topic Thread-Index)
         )
       (require 'epg)
       (setq
         mml2015-use 'epg
       ...
       (setq
         ...
         gnus-summary-make-false-root         'empty  ; add empty node when needing a root nod
         gnus-summary-make-false-root-always  nil     ; only if necessary
         gnus-summary-display-arrow           t 
         gnus-summary-thread-gathering-function 'gnus-gather-threads-by-references
         gnus-thread-hide-subtree             nil     ; expand threads
         gnus-build-sparse-threads            'some   ; fetch some threads to get better threads
       ...
       (setq
         ...
         gnus-summary-line-format (concat
                                    "%z%U%R "
                                    "%~(max-right 17)~(pad-right 17)&user-date;  "
                                    "%~(max-right 20)~(pad-right 20)f "
                                    "%B%s\n")
         gnus-thread-sort-functions '(gnus-thread-sort-by-most-recent-date)
         gnus-group-line-format "%ue%uM %S%p %P%5y:%B%(%g%)%O\n"
       ...
       (add-hook 'gnus-after-getting-new-news-hook
         (defun sf/gnus-sort-groups-after-new-news ()
       ...
       (defun gnus-user-format-function-e (dummy)
         "Display the unread mark in group line"
         (char-to-string gnus-unread-mark))

       (defun gnus-user-format-function-M (dummy)
         "Display the ticked mark in group line"
         (char-to-string gnus-ticked-mark))

       (defun sf/newsgroup-prefix (grp)
         "Return prefix of gnus newsgroup name"
         (concat (car (split-string grp ":")) ":"))

       (defun sf/email-trash ()
         "Trash the current or marked emails"
         (interactive)
         (gnus-summary-move-article nil (concat (sf/newsgroup-prefix gnus-newsgroup-name) "Trash"))
         (gnus-summary-next-article))

       (defun sf/email-archive ()
         "Archive the current or marked emails"
         (interactive)
         (gnus-summary-move-article nil (concat (sf/newsgroup-prefix gnus-newsgroup-name) "Archive"))
         (gnus-summary-next-article))
   ...
   (use-package bbdb
     ...
     (progn
       ...
       (setq
         bbdb-file (concat user-emacs-directory "user/bbdb")
...
Gnus starts in the [[http://www.gnus.org/manual/gnus_11.html#Group-Buffer][group buffer]].  This lists subscribed groups.  For an
email account, this would be email folders.

| Keybinding | Command                              | Description                             |
|------------+--------------------------------------+-----------------------------------------|
| A A        | gnus-group-list-active               | List all groups available from server   |
| L          | gnus-group-list-all-groups           | List all groups with level ARG or lower |
| u          | gnus-group-unsubscribe-current-group | Toggle subscription to current GROUP    |
| t          | gnus-topic-mode                      | Show topics                             |
| g          | gnus-group-get-new-news              | Get newly arrived articles              |

*Summary*

The [[http://www.gnus.org/manual/gnus_30.html#Summary-Buffer][summary buffer]] displays a line for each article.  You can move
around, read articles, reply, and so on.

| Keybinding | Command                          | Description                                  |
|------------+----------------------------------+----------------------------------------------|
| =          | gnus-summary-expand-window       | Make the summary buffer take up entire frame |
| n          | gnus-summary-next-unread-article | Select unread article after current one      |
| p          | gnus-summary-prev-unread-article | Select unread article before current one     |
| N          | gnus-summary-next-article        | Select article after current one             |
| P          | gnus-summary-prev-article        | Select article before current one            |
| / w        | gnus-summary-pop-limit           | Remove search limit                          |
|            |                                  |                                              |
